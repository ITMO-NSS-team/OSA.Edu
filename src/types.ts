export type Provider = "gemini" | "openrouter" | "host";
...
export interface Health { ok: boolean; models: ModelInfo[]; configured: { gemini: boolean; openrouter: boolean; host: boolean }; host?: { installed: boolean; authenticated: boolean; path?: string | null; detail: string }; defaults: { prompt: string; mapPrompt: string; additionalCriteria: string; profile: CheckProfile }; knowledge: { coreCount: number; softCount: number; fullCount: number; retrieval: string }; rateLimits: { gemini: Record<string, number>; openrouter: Record<string, number>; host: Record<string, number> }; normcontrol: { mcpUrl: string; dagId: string; mcpAttempts?: number; mcpAttemptTimeoutSeconds?: number; enabled: boolean }; }
