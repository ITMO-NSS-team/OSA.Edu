...
function Report({ job, status, query, onStatus, onQuery, onAction }: { job: Job; status: StatusFilter; query: string; onStatus: (value: StatusFilter) => void; onQuery: (value: string) => void; onAction: Props["onAction"] }) {
  ...
  return <div>
    ...
    <details className="panel technical-details"><summary>Промпты, маршрутизация и API</summary><div className="technical-body">
      ...
      <p><b>Маршрутизация:</b> {report.routing.strategy}; явно задано правил: {report.routing.explicitRules}; fallback: {report.routing.fallbackRules}; фрагментов: {report.routing.fragments}; физических запросов проверки: {report.routing.physicalRequests ?? report.routing.checkRequests}; план первого прохода: {report.routing.plannedCheckRequests ?? report.routing.checkRequests}; candidate: {report.routing.candidateRequests ?? 0}; semantic first-pass: {report.routing.semanticRequests ?? 0}; сокращения: Python; evidence verifier: {report.routing.evidenceVerifierRequests ?? 0}</p>
