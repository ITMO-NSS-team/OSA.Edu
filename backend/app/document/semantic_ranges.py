...
def trim_blocks_for_element(element_type: str, blocks: list[dict]) -> list[dict]:
    ...
    if element_type == "defense_statements":
        return _trim_between(blocks, re.compile(
            ...
            re.compile(r'(?:^|\n|[.!?]\s+)(?:Достоверность(?:\s+научных\s+результатов)?|Степень\s+достоверности|Апробация\s+работы|Личный\s+вклад\s+автора|Методологическ\p{L}*\s+основ\p{L}*\s+работы|Практическая\s+значимость(?:\s+работы)?|Теоретическая\s+значимость(?:\s+работы)?|Публикации|Структура\s+и\s+объ[её]м)\s*\.?', re.I)
        ])
