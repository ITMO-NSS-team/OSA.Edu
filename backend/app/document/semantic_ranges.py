...
def trim_blocks_for_element(element_type: str, blocks: list[dict]) -> list[dict]:
    ...
    if element_type == "tasks":
        return _trim_between(blocks, re.compile(r'(?:^|\n|[.!?]\s+)Задачи\s+(?:диссертационной\s+)?работы\s*\.?', re.I), [
            re.compile(r'(?:^|\n|[.!?]\s+)(?:Объект\s+и\s+предмет\s+исследования|Научная\s+новизна|Теоретическая\s+и\s+практическая\s+значимость|Положения,?\s+выносимые\s+на\s+защиту)\s*\.?', re.I)
        ])
    if element_type == "defense_statements":
        return _trim_between(blocks, re.compile(r'(?:^|\n|[.!?]\s+)Положения,?\s+выносимые\s+на\s+защиту\s*\.?', re.I), [
            re.compile(r'(?:^|\n|[.!?]\s+)(?:Достоверность\s+научных\s+результатов|Апробация\s+работы|Личный\s+вклад\s+автора|Публикации|Структура\s+и\s+объ[её]м)\s*\.?', re.I)
        ])
